...
def darknet53_whole(input_data, trainable):
    ...
    with tf.variable_scope("darknet"):
        ...
        input_data = common.convolutional(
            input_data, filters_shape=(3, 3, 3, 32), trainable=trainable, name="conv0"
        )
        input_data = common.convolutional(
            input_data,
        ...
        for i in range(1):
            input_data = common.residual_block(
                input_data, 64, 32, 64, trainable=trainable, name="residual%d" % (i + 0)
            )
...
def darknet53(input_data, trainable):
    ...
    with tf.variable_scope("darknet"):
        ...
        input_data = common.convolutional(
            input_data, filters_shape=(3, 3, 3, 12), trainable=trainable, name="conv0"
        )
        input_data = common.convolutional(
            input_data,
            filters_shape=(3, 3, 12, 24),
            trainable=trainable,
            name="conv1",
            downsample=True,
        )
